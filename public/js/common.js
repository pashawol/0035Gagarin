...
function eventHandler() {
	...
	function animateTextVertical(text, parent, y = "-=200%") {
		var scene = new ScrollMagic.Scene({
			triggerElement: parent,
			duration: 1000
		}).setPin("".concat(parent, " ").concat(text)).setTween(tween) // .addIndicators() // add indicators (requires plugin)
		.addTo(controller);
	...
	var tween = TweenMax.to(".sAbout__row", 300, {
		x: '-=50%'
	}); // build scene
	...
	.addTo(controller); // animateText('.sAdvantages__before', '.sAdvantages');
	...
	window.addEventListener('scroll', function (e) {
		...
		isScrollUp = newScroll <= oldScroll && newScroll > height; // console.log(height);
